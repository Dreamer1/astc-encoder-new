...
TEST(CompressAstcTest, BasicTest) {
    // Load the image
    std::string filename = "/Users/admin/Downloads/astc-encoder-4.7.0/Source/CompressTest/dou.JPG";
...
//    }
    ...
    uint8_t* output_image_raw = new uint8_t[5529616];
    uint32_t width = 3840;
    uint32_t height = 5760;
    uint32_t block_width = 8;
    uint32_t block_height = 8;
    float quality = 30;
    uint32_t is_srgb = 0;
    unsigned int repeat_count = 12;
    ...
    output_file.write(reinterpret_cast<char*>(output_image_raw), 5529616);
    output_file.close();
