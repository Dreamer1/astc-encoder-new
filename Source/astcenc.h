...
ASTCENC_PUBLIC astcenc_error compress_astc(uint8_t *in,size_t image_len,char* format, uint8_t *out, uint32_t width,
 ...
 ASTCENC_PUBLIC astcenc_image* load_ncimage_from_memory(
        char* eptr,
        uint8_t* data,
        size_t size,
        int y_flip
    );
  ASTCENC_PUBLIC astcenc_header get_astcenc_header(astcenc_image* image, unsigned int block_x, unsigned int block_y, unsigned int block_z);
  const uint32_t ASTCENC_MAGIC_ID = 0x5CA1AB13;
  ASTCENC_PUBLIC astcenc_image* astc_img_from_unorm8x4_array(
        const uint8_t* data,
        unsigned int dim_x,
        unsigned int dim_y,
        bool y_flip);
ASTCENC_PUBLIC astcenc_image* alloc_image(
    unsigned int bitness,
    unsigned int dim_x,
    unsigned int dim_y,
    unsigned int dim_z);
 ASTCENC_PUBLIC astcenc_image* load_png_with_wuffs_from_memory(
     const uint8_t* buffer,
     size_t size,
     bool y_flip
    );

    ASTCENC_PUBLIC astcenc_image* load_image_from_memory(
     const uint8_t* buffer,
     size_t size,
     bool y_flip
    );
